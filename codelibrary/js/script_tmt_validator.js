...
// use onblur="extractNumber(this,2,true);" onkeyup="extractNumber(this,2,true);" onkeypress="return blockNonNumbers(this, event, true, true);" 
...
tmt_globalPatterns.url = new RegExp("^[\\w\\.=-]+\\.[\\w\\.-]+\\.[\\w\\.-]{2,4}$");
...
function del_prompt(frmobj,comb,destUrl)
{
	...
	if(comb=='Assign Customer'){
       if(frmobj.user_id.value==''){
...
tmt_globalRules.greaterthan = function(fieldNode) {
  ...
  var datevar1 = new Date(date1Arr[0],date1Arr[1],date1Arr[2]);
  var datevar2 = new Date(date2Arr[0],date2Arr[1],date2Arr[2]);

 return datevar1<=datevar2;
...
tmt_globalRules.greaterthancurrentdate = function(fieldNode) {
	...
	date2Arr = fieldNode.value.split("-");
	var datevar2 = new Date(date2Arr[0],date2Arr[1]-1,date2Arr[2]);
	dateobj = new Date();
	var datevar1 = new Date(dateobj.getFullYear(),dateobj.getMonth(),dateobj.getDate());
	//alert(datevar2+":"+datevar1);
